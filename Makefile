...
SIMAPP = ./app
HTTPS_GIT := https://github.com/novic-labs/novic.git
DOCKER := $(shell which docker)
DOCKER_BUF := $(DOCKER) run --rm -v $(CURDIR):/workspace --workdir /workspace bufbuild/buf
NAMESPACE := novicnetwork
PROJECT := novic
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=novic \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(NOVIC_BINARY) \
...
build-docker:
	...
	$(DOCKER) rm novic || true
	# create a new container from the latest image
	$(DOCKER) create --name novic -t -i ${DOCKER_IMAGE}:latest novic
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp novic:/usr/bin/novicd ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/novic-labs/novic/types"
	godoc -http=:6060
...
test-sim-custom-genesis-fast:
	...
	@echo "By default, ${HOME}/.$(NOVIC_DIR)/config/genesis.json will be used."
	@go test -mod=readonly $(SIMAPP) -run TestFullAppSimulation -Genesis=${HOME}/.$(NOVIC_DIR)/config/genesis.json \
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.$(NOVIC_DIR)/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.$(NOVIC_DIR)/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 400 5 TestFullAppSimulation
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(NOVIC_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/novic:Z novic/node "./novicd testnet init-files --v 4 -o /novic --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\novicd:/novic\Z novic/node "./novicd tendermint unsafe-reset-all --home=/novic"
	@docker run --rm -v $(CURDIR)\build\node1\novicd:/novic\Z novic/node "./novicd tendermint unsafe-reset-all --home=/novic"
	@docker run --rm -v $(CURDIR)\build\node2\novicd:/novic\Z novic/node "./novicd tendermint unsafe-reset-all --home=/novic"
	@docker run --rm -v $(CURDIR)\build\node3\novicd:/novic\Z novic/node "./novicd tendermint unsafe-reset-all --home=/novic"
else
	@docker run --rm -v $(CURDIR)/build/node0/novicd:/novic:Z novic/node "./novicd tendermint unsafe-reset-all --home=/novic"
	@docker run --rm -v $(CURDIR)/build/node1/novicd:/novic:Z novic/node "./novicd tendermint unsafe-reset-all --home=/novic"
	@docker run --rm -v $(CURDIR)/build/node2/novicd:/novic:Z novic/node "./novicd tendermint unsafe-reset-all --home=/novic"
	@docker run --rm -v $(CURDIR)/build/node3/novicd:/novic:Z novic/node "./novicd tendermint unsafe-reset-all --home=/novic"
endif
...
PACKAGE_NAME:=github.com/novic-labs/novic
