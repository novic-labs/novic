...
// along with the Evmos packages. If not, see https://github.com/novic-labs/novic/blob/main/LICENSE
...
import (
	...
	"github.com/novic-labs/novic/v2/server/config"
	novictypes "github.com/novic-labs/novic/v2/types"
...
func DefaultConfig() Config {
	...
	chainId := fmt.Sprintf("novic_%d-1", tmrand.Int63n(9999999999999)+1)
	return Config{
		Codec:             encCfg.Codec,
		...
		NumValidators:     4,
		BondDenom:         "anovic",
		MinGasPrices:      fmt.Sprintf("0.000006%s", novictypes.AttoPhoton),
		AccountTokens:     sdk.TokensFromConsensusPower(1000000000000000000, novictypes.PowerReduction),
		StakingTokens:     sdk.TokensFromConsensusPower(500000000000000000, novictypes.PowerReduction),
		BondedTokens:      sdk.TokensFromConsensusPower(100000000000000000, novictypes.PowerReduction),
...
func New(l Logger, baseDir string, cfg Config) (*Network, error) {
	...
	if !novictypes.IsValidChainID(cfg.ChainID) {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		nodeDir := filepath.Join(network.BaseDir, nodeDirName, "novicd")
		clientDir := filepath.Join(network.BaseDir, nodeDirName, "noviccli")
		...
		genAccounts = append(genAccounts, &novictypes.EthAccount{
		...
		customAppTemplate, _ := config.AppConfig(novictypes.AttoPhoton)
