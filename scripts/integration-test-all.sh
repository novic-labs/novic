...
init_func() {
    "$PWD"/build/novicd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/novicd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/novicd add-genesis-account \
    "$("$PWD"/build/novicd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000anovic,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/novicd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/novicd collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/novicd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/novicd start --pruning=nothing --rpc.unsafe \
    ...
    NOVIC_PID=$!
    echo "started novic node, pid=$NOVIC_PID"
    # add PID to array
    arr+=("$NOVIC_PID")
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test novic node $HOST_RPC ..."
...
stop_func() {
    NOVIC_PID=$i
    echo "shutting down node, pid=$NOVIC_PID ..."
