...
import (
	...
	cryptocodec "github.com/novic-labs/novic/v2/crypto/codec"
	enccodec "github.com/novic-labs/novic/v2/encoding/codec"
...
func TestKeyring(t *testing.T) {
	...
	info, mnemonic, err := kr.NewMnemonic("foo", keyring.English, novictypes.BIP44HDPath, keyring.DefaultBIP39Passphrase, EthSecp256k1)
	require.NoError(t, err)
...
func TestDerivation(t *testing.T) {
	bz, err := EthSecp256k1.Derive()(mnemonic, keyring.DefaultBIP39Passphrase, novictypes.BIP44HDPath)
	require.NoError(t, err)
	require.NotEmpty(t, bz)
	...
	path := MustParseDerivationPath(novictypes.BIP44HDPath)
	account, err := wallet.Derive(path, false)
	require.NoError(t, err)
