...
import (
	...
	"github.com/novic-labs/novic/v2/testutil/network"
)
...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		accStakingTokens := sdk.TokensFromConsensusPower(5000, novictypes.PowerReduction)
		coins := sdk.Coins{
			sdk.NewCoin(novictypes.AttoPhoton, accStakingTokens),
		}
		...
		valTokens := sdk.TokensFromConsensusPower(100, novictypes.PowerReduction)
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(novictypes.AttoPhoton, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(novictypes.AttoPhoton)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, novictypes.AttoPhoton, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
