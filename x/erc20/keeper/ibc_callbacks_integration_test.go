...
import (
	...
	"github.com/novic-labs/novic/v2/contracts"
	ibctesting "github.com/novic-labs/novic/v2/ibc/testing"
	teststypes "github.com/novic-labs/novic/v2/types/tests"
	"github.com/novic-labs/novic/v2/utils"
	"github.com/novic-labs/novic/v2/x/erc20/types"
...
var _ = Describe("Convert receiving IBC to Erc20", Ordered, func() {
	...
	evmosMeta := banktypes.Metadata{
		...
		DenomUnits: []*banktypes.DenomUnit{
			{
				Denom:    teststypes.AnovicDenomtrace.BaseDenom,
				Exponent: 0,
			},
		},
		Name:    utils.BaseDenom,
		Symbol:  erc20Symbol,
		Display: teststypes.AnovicDenomtrace.BaseDenom,
	}
	...
	Describe("enabled params and registered uosmo", func() {
		...
		It("should transfer and not convert aevmos", func() {
			...
			aevmosIBCBalanceBefore := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), senderAcc, teststypes.AnovicIbcdenom)
			s.Require().Equal(amount, aevmosIBCBalanceBefore.Amount.Int64())

			// 2. Send aevmos IBC coins from Osmosis to Evmos
			ibcCoinMeta := fmt.Sprintf("%s/%s", teststypes.AnovicDenomtrace.Path, teststypes.AnovicDenomtrace.BaseDenom)
			s.SendBackCoins(s.pathOsmosisEvmos, s.IBCOsmosisChain, teststypes.AnovicIbcdenom, amount, sender, receiver, 1, ibcCoinMeta)

			// check ibc aevmos coins balance on Osmosis - should be zero
			aevmosIBCSenderFinalBalance := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), senderAcc, teststypes.AnovicIbcdenom)
			...
			ibcCoinsBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, teststypes.AnovicIbcdenom)
...
var _ = Describe("Convert outgoing ERC20 to IBC", Ordered, func() {
	...
	Describe("disabled params", func() {
		...
		It("should fail transfer and not convert to IBC", func() {
			...
			denom := originChain.App.(*app.NovicApp).StakingKeeper.BondDenom(originChain.GetContext())
